...
const ast_mongo_1 = require("../src/ast_mongo");
//
//
//
...
describe('AstMongo.ts connection & disconnection', () => {
    test('construction', () => {
        ast_mongo = new ast_mongo_1.AstMongo(astMongoOptions);
    });
    test('connect', async () => {
...
describe('AstMongo.ts with Aor', () => {
    ...
    describe('aor', () => {
        ...
        test('Update it', async () => {
            const result = await ast_mongo.Aor.update({ _id: 'alice' }, { max_contacts: 2 });
            expect(result).toMatchSnapshot();
        });
...
describe('AstMongo.ts with Endpoint Set', () => {
    beforeAll(async () => {
        ...
        esh = new ast_mongo_1.EndpointSetHelper(ast_mongo);
    });
...
describe('AstMongo.ts with StaticConfig', () => {
    beforeAll(async () => {
        ...
        smh = new ast_mongo_1.StaticModelHelper(ast_mongo.Static, unique_id);
    });
...
//# sourceMappingURL=ast_mongo_ts.test.js.map